...
export interface RealtimeAuditViewProps {
  context: WebPartContext;
  sp: SharePointService;
  onBack: () => void;
}

export const RealtimeAuditView: React.FC<RealtimeAuditViewProps> = ({ context, sp, onBack }) => {
  const styles = useStyles();
  ...
  // ── Tab ──
  const [tab, setTab] = React.useState<'browse' | 'user'>('browse');
  ...
  const handleConnect = async (): Promise<void> => {
    ...
    try {
      const [libs, users] = await Promise.all([
        sp.getLibraries(siteUrl.trim(), abortRef.current.signal),
        sp.getSiteUsers(siteUrl.trim(), abortRef.current.signal),
      ]);
  ...
  const loadChildren = async (node: FolderFileNode): Promise<void> => {
    node.isLoading = true;
    setRootNodes((prev) => [...prev]); // force re-render
  ...
  // Re-load when expandGroups changes
  React.useEffect(() => {
    ...
    <div className={styles.root}>
      ...
      {isConnected && (
        <>
          <TabList
            selectedValue={tab}
            onTabSelect={(_, d) => setTab(d.value as 'browse' | 'user')}
            style={{ marginBottom: tokens.spacingVerticalM }}
          >
            <Tab value="browse">📁 Folder / File</Tab>
            <Tab value="user">👤 User Access</Tab>
          </TabList>
